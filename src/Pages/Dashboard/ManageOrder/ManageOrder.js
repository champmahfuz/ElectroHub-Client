...
const ManageOrder = () => {
    ...
    useEffect(() => {
        fetch('https://electro-hub-server.onrender.com/orders')
            .then(res => res.json())
    ...
    /* 
        const handleUpdateStatus = productId => {
            fetch(`https://electro-hub-server.onrender.com/orders/${productId}`, {
                method: 'PUT',
                headers: {
    ...
    const handleDeleteOrder = orderId => {
        ...
        if (deleteConfirmation) {
            const productUri = `https://electro-hub-server.onrender.com/orders/${orderId}`;
            fetch(productUri, {
                method: 'DELETE'
            })
                .then(res => res.json())
